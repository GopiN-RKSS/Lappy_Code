...
class DiscountsList extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (this.state.discounts.length === 0) {
      ...
      this.state.filterDiscount =
        nextProps.discountsList.discountslist.discounts.slice(
          this.state.offset,
          this.state.offset + this.state.perPage
        );
      const postData = this.state.filterDiscount.map((pd) => (
  ...
  receivedData() {
    this.state.slice = this.state.discounts.slice(
      this.state.offset,
      this.state.offset + this.state.perPage
    );
  ...
  searchHandler = (event) => {
    if (this.state.DiscountSearchInput === 0) {
      this.props.getDiscounts();
      ...
      if (displayedContacts.length > 0) {
        this.setState({
          DefaultMessage: "",
          DiscountSearchList: displayedContacts,
          pageCount: Math.ceil(displayedContacts.length / this.state.perPage),
          DiscountSearchInput: event.target.value,
        });
      } else {
        this.setState({
          DiscountSearchList: [],
            ...
            <div style={{ width: "100%", float: "right" }}>
              ...
              >
                ...
                <input
                  ...
                  placeholder="Search Discount Program..."
                ...
                <div className="mb-3 col-12 text-center">
                  <div className="table-responsive">
                    <table id="dataTable">
                      ...
                      <tbody>
                        {this.state.DiscountSearchInput.length ? (
                          this.state.DiscountSearchList.length ? (
                            this.state.DiscountSearchList?.slice(
                              this.state.offset,
                              this.state.offset + this.state.perPage
                            ).map((item, index) => (
...
export default connect(mapStateToProps, { getDiscounts, deleteDiscountForm })(
  DiscountsList
);
